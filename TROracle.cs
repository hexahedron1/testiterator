...
public class TRGeneralConversation(SSOracleBehavior owner, Conversation.ID convoID) : 
    SSOracleBehavior.ConversationBehavior(owner, SubBehavID.MeetWhite, convoID)
{
    public float PartialGravity;
    ...
    public bool LockPaths;
    public bool SeenPlayer;
    public bool ChangePassiveBehavior;
    public bool ActiveNeuronMovement;
    public bool ActiveBotMovement;
    ...
    public virtual Vector2 GrabPos
    {
        ...
        {
            Oracle oracle = this.oracle;
            return !(oracle.graphicsModule is TROracleGraphics graphicsModule) ? oracle.firstChunk.pos : graphicsModule.hands[1].pos;
        }
    }
    ...
    public virtual bool SpearImage
    {
        get
        {
            return ModManager.MSC && oracle.room.game.StoryCharacter == MoreSlugcatsEnums.SlugcatStatsName.Spear;
        }
    }

    public virtual bool SeerImage
    {
        get
        {
            string a = oracle.room.game.StoryCharacter?.value;
            return a != null && string.Equals(a, "seer", StringComparison.OrdinalIgnoreCase);
        }
    }
    ...
    {
        SSOracleBehavior owner = this.owner;
        Room room = owner.oracle.room;
        Vector2 vector2 = new Vector2(Random.value * room.PixelWidth, Random.value * room.PixelHeight);
        if (owner.CommunicatePosScore(vector2) + 40.0 < owner.CommunicatePosScore(owner.nextPos) && !Custom.DistLess(vector2, owner.nextPos, 30f))
            owner.SetNewDestination(vector2);
        ConsistentShowMediaPosCounter += (int) Custom.LerpMap(Vector2.Distance(ShowMediaPos, IdealShowMediaPos), 0.0f, 200f, 1f, 10f);
        vector2 = new Vector2(Random.value * room.PixelWidth, Random.value * room.PixelHeight);
        if (ShowMediaScore(vector2) + 40.0 < ShowMediaScore(IdealShowMediaPos))
        {
            IdealShowMediaPos = vector2;
            ConsistentShowMediaPosCounter = 0;
        }
        vector2 = IdealShowMediaPos + Custom.RNV() * Random.value * 40f;
        if (ShowMediaScore(vector2) + 20.0 < ShowMediaScore(IdealShowMediaPos))
        {
            IdealShowMediaPos = vector2;
            ConsistentShowMediaPosCounter = 0;
        }
    ...
    public virtual float ShowMediaScore(Vector2 tryPos)
    {
        ...
        if (!oracle.room.GetTile(tryPos).Solid)
        {
            ...
            if (player != null)
            {
                float num = Math.Abs(Vector2.Distance(tryPos, player.DangerPos) - 250f) - Math.Min((float) oracle.room.aimap.getTerrainProximity(tryPos), 9f) * 30f - Vector2.Distance(tryPos, owner.nextPos) * 0.5f;
                foreach (Oracle.OracleArm.Joint joint in oracle.arm.joints)
                    num -= Math.Min(Vector2.Distance(tryPos, joint.pos), 100f) * 10f;
                if (oracle.graphicsModule is TROracleGraphics graphicsModule)
                {
                    Vector2[,] coord = graphicsModule.umbCord.coord;
                    int length = coord.GetLength(0);
                    for (int index = 0; index < length; index += 3)
                        num -= Math.Min(Vector2.Distance(tryPos, coord[index, 0]), 100f);
                }
                return num;
            }
        }
